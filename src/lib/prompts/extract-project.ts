...
\`\`\`json
{
  ...
  "suggestedDeliverables": ["assessment" | "architecture" | "bom" | "professional-services" | "tco" | "project-plan" | "proposal" | "sow" | "ms-offering"],
  "confidence": {
...
**Assessment leads every applicable flow** — its Executive Summary now opens with a customer-background briefing (industry context, IT landscape, regulatory posture, stakeholders) so the team gets the engagement-priming context AND the readiness scoring in one deliverable. Optional add-ons SOW + ms-offering only when the engagement clearly calls for them.
