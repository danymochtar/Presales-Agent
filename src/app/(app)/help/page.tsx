      ...
      <Card>
        ...
        <CardContent>
          <ul className="text-sm space-y-1.5 list-disc pl-5">
            ...
            <li><strong>CRM connector (Creatio):</strong> if you keep your live pipe in Creatio, configure the connector on <Link href="/admin" className="underline">/admin</Link> instead of exporting Excel every week. Provide your Creatio base URL + a dedicated API user; click <em>Test connection</em>, then <em>Sync opportunities now</em>. The agent does cookie-auth against <code>/ServiceModel/AuthService.svc/Login</code> then pulls OData v4 from <code>/0/odata/Opportunity</code>. Syncs land in a tracker named &quot;Creatio CRM&quot; tagged as a CRM-sync — re-running upserts by Creatio Id so no duplicates.</li>
