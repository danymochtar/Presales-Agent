...
export default async function AssessmentPage({
  ...
  const project = await prisma.engagement.findFirst({
    ...
    include: {
      ...
      deliverables: { where: { type: "assessment" }, orderBy: { version: "desc" } },
    },
  });
  if (!project) notFound();
  ...
  return (
    <div className="space-y-4">
      ...
      generatePath={`/api/engagements/${project.id}/assessment/generate?cloud=${activeCloud}`}
      canGenerate={hasInventory}
      prerequisiteMessage={hasInventory ? null : "Upload an inventory (RVTools / Azure Migrate / generic CSV) first — assessment scores per-workload readiness."}
