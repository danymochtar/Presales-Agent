...
const EngagementTypeEnum = z.enum(["migration", "greenfield", "modernization", "dr", "poc", "optimization", "unknown"]);
...
const CreateProject = z.object({
  ...
  engagementTypeConfidence: z.enum(["high", "medium", "low"]).optional(),
  engagementTypeRationale: z.string().optional(),
...
export async function POST(req: NextRequest) {
  ...
  const project = await prisma.$transaction(async (tx) => {
    ...
    if (inputs && inputs.length > 0) {
      await tx.engagementInput.createMany({
        data: inputs.map((i) => ({
