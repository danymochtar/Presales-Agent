...
export async function POST(_req: NextRequest, ctx: { params: Promise<{ id: string; inputId: string }> }) {
  ...
  const input = await prisma.engagementInput.findFirst({
    where: { id: inputId, engagementId: id, engagement: { tenant: { users: { some: { id: session.user.id } } } } },
    include: { engagement: { select: { tenantId: true } } },
  });
  ...
  try {
    ...
    const updated = await prisma.engagementInput.update({
      where: { id: input.id },
      data: {
